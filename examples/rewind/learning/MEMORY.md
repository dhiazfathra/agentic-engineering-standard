...
- 2026-09-23 Seeded Rewinds have no real object in MinIO (seed has no S3 dependency); the viewer must render a missing-media state for them. `seed()` upserts by fixed id (`onConflictDoUpdate`) instead of delete-then-insert, so re-seeding no longer cascades away user comments/events on seeded Rewinds or nulls a user Rewind's `folderId` via the folders FK's `ON DELETE SET NULL`.
