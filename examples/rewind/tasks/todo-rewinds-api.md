...
- [x] `packages/schema` depends on `zod` (same version as `apps/web`) and
      ...
      inferred types.
- [x] `createRewind` rejects a `mediaKey` that does not match
      `mediaKeyPattern`, and a screenshot with a duration.
- [x] `packages/schema` has `test` (Vitest, 100% thresholds) and
...
- [x] `folders`, `recordingLinks`, `rewinds`, `events`, `comments` match
      the spec's data model. Ids are text with `$defaultFn(newId)`.
      Timestamps are integer `timestamp` mode.
- [x] `events` and `comments` cascade on Rewind delete. `rewinds.folderId`
      and `rewinds.recordingLinkId` set null on parent delete.
- [x] Drizzle `relations` let `db.query.rewinds.findFirst({ with: { events, comments } })` work.
- [x] Migration `0001_*` comes from `bun run --filter web db:generate`,
      not hand-written.
...
- [x] Parses `uploadRequest`, returns `{ url, key }`: a presigned
      `PutObjectCommand` URL, 15-minute TTL, content type signed in, key
      `rewinds/<newId()>.<ext>`.
- [x] 400 on a bad body. Unit test with the presigner stubbed.

**Dependencies:** T1 · **Scope:** S

## T4: `/api/rewinds` GET and POST

- [x] GET lists Rewinds newest first (row columns only; pagination is
      deferred to `library`).
- [x] POST parses `createRewind`, inserts the Rewind and its events in
...
- [x] GET returns the Rewind with `events` and `comments`, each sorted by
      `t`. 404 when missing.
- [x] PATCH parses `updateRewind`, bumps `updatedAt`. 404 missing, 400 bad
      body or unknown folder.
- [x] DELETE removes the row (cascade), then tries `DeleteObjectCommand`
...
- [x] Parses `createComment`, returns 201 with the row. 404 when the Rewind
...
- [x] GET lists, POST creates (201). PATCH renames, DELETE deletes; both
...
- [x] `apps/web/src/db/seed.ts`, run by `bun run db:seed` (root and
      ...
      rows.
- [x] The row-building logic is unit tested; the entry point stays thin.

**Dependencies:** T2 · **Scope:** S

## T10: Integration test

- [x] One Vitest suite runs the real route handlers against a migrated
...
- [x] `apps/web/e2e/rewinds.spec.ts`: POST `/api/uploads`, `PUT` a small
...
- [x] `docs/STACK.md` lists `db:seed` and the new routes directory.
