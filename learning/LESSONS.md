...
- 2026-09-24 [project] `fm-ensure-agents-md.sh` replaces a `CLAUDE.md -> AGENTS.md` symlink with an `@AGENTS.md` pointer file. Evidence: running it at the root turned `CLAUDE.md` into a 90-byte file, and git showed a typechange. Do instead: after running it, check `ls -la CLAUDE.md` and restore the symlink with `ln -sf AGENTS.md CLAUDE.md`; the symlinks are this repo's design.
